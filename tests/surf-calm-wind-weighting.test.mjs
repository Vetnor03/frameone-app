...
test('app forecast and physical frame scoring flow through the shared surf scoring helper', () => {
  ...
  assert.match(surfRoute, /aggregation: 'best_scored_hour_same_as_frame_dayparts'/)
  assert.match(surfRoute, /function scoreSurfBucketAtIdx[\s\S]*?scoreSurf\(/)
  assert.match(surfRoute, /function appForecastDaypartBucket[\s\S]*?bestScoredSurfBucketAroundTargetIso/)
