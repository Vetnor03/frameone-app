...
test('one localized helper is reused for user-created free text', () => {
  ...
  assert.match(helper, /Av hensyn til sikkerheten og personvernet ditt bør du ikke dele passord, betalingsopplysninger, fødselsnummer, helseopplysninger/)
  ...
  const dinnerPlanSheet = app.slice(app.indexOf('function DinnerPlanSheet'), app.indexOf('function DinnerPlanAddItemSheet'))
  ...
  assert.doesNotMatch(dinnerPlanSheet, /SensitiveInformationHelper/)
