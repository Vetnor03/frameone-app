...
test('compact frame schema preserves only the optional adaptive Weather facts it consumes', () => {
  ...
  assert.equal(compactFields, "'time', 'temperature_2m', 'weather_code', 'wind_speed_10m', 'precipitation_probability', 'precipitation'")
  for (const field of ['apparent_temperature', 'wind_gusts_10m']) {
...
test('firmware JSON filter explicitly permits only consumed weather fields and insight', () => {
  ...
  assert.match(firmware, /filter\["hourly"\]\["precipitation_probability"\] = true/)
  for (const field of ['apparent_temperature', 'wind_gusts_10m']) {
