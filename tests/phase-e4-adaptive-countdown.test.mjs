...
test('firmware adaptive capability and custom preflight accept exact Countdown bases',async()=>{
 ...
 const preflight=layout.slice(layout.indexOf('static bool prepareCustomRender'),layout.indexOf('void drawWithContent'))
 assert.match(preflight,/ModuleRenderer::canRenderCell\(module, cell\)[\s\S]*output = staged;[\s\S]*return true/)
 assert.match(layout,/customReady = key == LAYOUT_CUSTOM && prepareCustomRender[\s\S]*!customReady \? LAYOUT_DEFAULT : key/)
})
