...
import { parseReminder, reminderParseJsonSchema, validateParsedReminder, REMINDER_PARSE_VERSION } from '../app/lib/reminders/parser.ts'
...
for (const [name, text, language, expected] of [
  ...
  ['weekly recurrence', 'Put out the bins every Sunday', 'en', { ...base, title: 'Put out the bins', due_date: '2026-08-23', due_time: null, repeat_type: 'weekly' }],
  ...
  ['missing time remains null', 'Call Dad tomorrow', 'en', { ...base, title: 'Call Dad', due_date: '2026-08-20', due_time: null }],
  ['ambiguous later invents nothing', 'Call Dad later', 'en', { ...base, title: 'Call Dad', due_date: null, due_time: null, ambiguities: ['When?'] }],
...
test('real calendar dates accept valid leap and ordinary days and reject impossible dates', () => {
  ...
  assert.equal(validateParsedReminder({ ...base, due_date: '2026-04-31' }), null)
  assert.deepEqual(validateParsedReminder({ ...base, due_date: '2028-02-29' }), { ...base, due_date: '2028-02-29' })
  assert.deepEqual(validateParsedReminder({ ...base, due_date: '2026-02-28' }), { ...base, due_date: '2026-02-28' })
  assert.equal(validateParsedReminder({ ...base, end_date: '2026-02-31' }), null)
})

test('parser schema only accepts fields represented by reminder creation', () => {
  assert.equal(validateParsedReminder({ ...base, note: 'remember insurance card' }), null)
  assert.equal(validateParsedReminder({ ...base, all_day: true }), null)
  assert.equal('note' in reminderParseJsonSchema.properties, false)
  assert.equal('all_day' in reminderParseJsonSchema.properties, false)
  const prompt = readFileSync(new URL('../app/lib/reminders/parser.ts', import.meta.url), 'utf8')
  assert.match(prompt, /meaningful reminder information[\s\S]*must remain in the reminder title/)
})
