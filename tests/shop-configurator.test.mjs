...
const imageNormalization = read('app/shop/configure/imageNormalization.ts')
...
test('preview layers normalize visible alpha bounds into one target per group', () => {
  assert.match(imageNormalization, /export const NORMALIZED_TARGETS/)
  assert.match(imageNormalization, /device: \{ centerX:/)
  assert.match(imageNormalization, /matte: \{ centerX:/)
  assert.match(imageNormalization, /frame: \{ centerX:/)
  assert.match(imageNormalization, /getImageData/)
  assert.match(imageNormalization, /\+ 3\] === 0/)
  assert.match(imageNormalization, /Math\.min\(/)
  assert.match(imageNormalization, /new Map<string, Promise<NormalizedImageMetadata>>/)
  assert.match(configurator, /<NormalizedLayer key=\{display\.id\}/)
  assert.match(configurator, /<NormalizedLayer key=\{matte\.id\}/)
  assert.match(configurator, /<NormalizedLayer key=\{frame\.id\}/)
  assert.match(configurator, /absolute inset-0/)
  assert.match(configurator, /opacity: transform \? 1 : 0/)
  assert.doesNotMatch(configurator, /object-contain/)
})
