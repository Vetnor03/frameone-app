...
test('AI Follow assignment survives editor split and history operations',()=>{const before=[{...initialLayout()[0],moduleId:'assistant'}],splitResult=splitCellAtBoundary(before,'cell',{axis:'vertical',boundary:1});assert.equal(splitResult.valid,true);assert.equal(splitResult.cells.filter(c=>c.moduleId==='assistant').length,1);const history=pushHistory(createHistory(before),splitResult.cells);assert.deepEqual(undoHistory(history).present,before);assert.deepEqual(redoHistory(undoHistory(history)).present,splitResult.cells)})
...
test('registry mirrors every firmware dispatch and drives the picker',async()=>{const registry=JSON.parse(await readFile(new URL('../shared/frame-modules.json',import.meta.url),'utf8')),renderer=await readFile(new URL('../frame/src/modules/ModuleRenderer.cpp',import.meta.url),'utf8'),simulator=await readFile(new URL('../app/frame-simulator/FrameSimulator.tsx',import.meta.url),'utf8');const expected=['date','weather','surf','reminders','countdown','soccer','stocks','groceries','assistant'];assert.deepEqual(registry.map(x=>x.id),expected);for(const id of expected.filter(id=>!['surf','assistant'].includes(id)))assert.match(renderer,new RegExp(`(?:equalsIgnoreCase|startsWith)\\("${id}`));assert.match(renderer,/exactOnly\(mod\.c_str\(\), "assistant"\)/);assert.match(renderer,/numericInstance\(mod\.c_str\(\), "surf"\)/);assert.match(simulator,/studioModuleRegistry\.map/);assert.doesNotMatch(simulator,/const modules\s*=/);assert.doesNotMatch(simulator,/committed\.forEach\(c=>ctx\.strokeRect/);assert.doesNotMatch(simulator,/ox\.strokeRect/);assert.match(simulator,/previewDividerStroke/);assert.match(simulator,/finalizeDividerStroke/);assert.doesNotMatch(simulator,/EditMode|setMode|>Draw<|>Erase</);for(const id of expected)assert.match(simulator,new RegExp(`${id}`))})
