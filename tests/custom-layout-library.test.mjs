...
test('inline editor exposes its dotted grid and reactive cell dividers',async()=>{
  ...
  assert.match(library,/finalizeDividerStroke\(cells,start,end/)
...
test('inline editor previews snapped drags without mutating geometry and safely cancels',async()=>{
  ...
  assert.match(editor,/const end=point\(e\),\{start\}=drag\.current,lock=drag\.current\.lock.*;clearDrag\(e\.pointerId\);const result=finalizeDividerStroke/)
