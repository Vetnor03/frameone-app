...
test('physical contract preserves blank assignments but rejects holes and incompatible assigned modules',()=>{
  const source=layout('layout-uuid','Four rows',0)
  const payload=customPhysicalPayload(source,{0:'date',1:null,2:'reminders'})
  assert.equal(payload.layout,'custom');assert.equal(payload.custom_layout_id,'layout-uuid')
  ...
  assert.ok(customPhysicalPayload({...source,cells:adaptive},{0:'date',1:null,2:'stocks'}))
})
test('empty module assignments are physical cells, not geometry holes',()=>{
  ...
  assert.equal(supportsPhysicalCustomLayout(adaptive.map((c,index)=>({...c,module:index===2?'stocks':''}))).valid,true)
})
...
test('physical support is explicitly module-aware',()=>{
  ...
  for(const module of ['reminders','reminders:1','reminders:calendar-id'])
    assert.equal(supportsPhysicalCustomCell({...cell(0,0,0,3,3),module}),true)
  assert.equal(supportsPhysicalCustomCell({...cell(0,0,0,3,3),module:'stocks'}),true)
  assert.equal(supportsPhysicalCustomCell({...cell(0,0,0,3,3),module:'groceries'}),true)
  for(const module of ['weatherfoo','weather-foo','notweather','remindersfoo','reminders-foo','groceries:1','groceriesfoo','ai-follow','unknown'])
...
test('custom creation and editing use the inline Frame tab state',async()=>{
  ...
  assert.match(page,/if\(!name\)\{setLayoutDraftError\('Enter a name for this layout\.'\);return\}/)
  assert.match(page,/if\(layoutFlow\?\.mode==='edit'\)[\s\S]*method:'PATCH'/)
  assert.doesNotMatch(page,/saveCustomLayout\(name:string,cells:CustomLayoutCell\[\]\) \{\s*if\(!activeDeviceId\)return/)
  assert.match(page,/if\(!activeDeviceId\)throw new Error\('Select a frame before creating a layout\.'\)/)
