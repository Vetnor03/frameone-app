#!/usr/bin/env python3
"""Create a flat Arduino sketch from the canonical RE:MIND frame firmware.

The repository keeps the firmware split into subdirectories for maintainability.
This helper creates a single Arduino sketch folder without duplicating maintained
firmware logic: it copies all source/header files from frame/src, rewrites only
repository-local quoted include paths to their flattened basenames, and installs
the checked-in Alfred V1.2 partition table as partitions.csv.
"""
...
def firmware_sources() -> list[Path]:
    ...
    if sketches != [expected_sketch]:
        ...
        raise RuntimeError(
            f"Expected only {expected_sketch.name} under {SOURCE_ROOT}; "
...
def prepare(output: Path) -> int:
    ...
    if output.name != SKETCH_NAME:
        raise RuntimeError(
            "Arduino requires the folder and main sketch to match; "
    ...
    if output.exists() and any(output.iterdir()):
        raise RuntimeError(
            ...
            "Use a new empty folder so stale files cannot be compiled."
        )
