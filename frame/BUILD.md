...
The existing PlatformIO profiles remain useful compatibility/regression builds, but they still resolve the older Arduino-ESP32 2.0.14 generation. They are **not** the Alfred production power build and must not be used to claim automatic connected light sleep is active.
...
The PlatformIO Alfred environment still resolves `boards/alfred_v1_2.json` for hardware regression coverage. That manifest declares ESP32-S3-WROOM-1-N16R8, 16 MiB flash, 80 MHz QIO flash mode, `qio_opi` Arduino memory type, OPI PSRAM with an expected size of 8 MiB, and the USB CDC/JTAG build flags. `platformio.ini` independently selects the checked-in `partitions_alfred_16mb.csv` table.
