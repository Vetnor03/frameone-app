#pragma once
#include <Arduino.h>

namespace WiFiManagerV2 {
  void begin();

  bool hasCreds();
  void clearCreds();

  // Try connect using saved creds (timeoutMs). The STA listen interval is
  // configured before association so MAX_MODEM can use the pilot 10-second
  // connected-idle target on power-management capable builds.
  bool connectSaved(uint32_t timeoutMs);

  // Apply the operational Wi-Fi/CPU power policy for the current power source.
  // Returns true when battery operation can remain connected using automatic
  // light sleep. A false return on battery means the caller must use the
  // 10-second deep-sleep probe fallback instead of staying awake. Set force
  // when another caller may have changed the Wi-Fi power-save mode.
  bool applyOperationalPowerPolicy(bool usbPresent, bool force = false);

  // Temporarily disable automatic light sleep / Wi-Fi power save while a
  // user-requested refresh downloads its content. The caller must restore the
  // source-aware operational policy immediately after the burst.
  bool beginRealtimeNetworkBurst();

  // Runtime telemetry for confirming whether battery operation is actually
  // staying in the connected automatic-light-sleep path or has fallen back
  // to timed deep sleep.
  const char* operationalPowerMode();

  // Save creds to flash
  void saveCreds(const String& ssid, const String& pass);

  // Read creds (for debugging)
  String getSsid();
}
