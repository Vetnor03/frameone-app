...
static void drawAdaptiveDate(const Cell& c, const char* month, const char* wday,
  ...
  // The physical 4x3 panel needs one generous calendar rather than two
  // compressed calendars. Keep the taller expanded layouts unchanged.
  const bool wideLargeSingleCalendar = c.w >= 700 && c.h >= 330 && c.h < 400;
  bool expanded = !wideLargeSingleCalendar && ((c.w >= 700 && c.h >= 330) || (c.w >= 520 && c.h >= 400));
  ...
  if (expanded) calendarSplit = true;
    ...
    calendarSplit = c.w * (wideLargeSingleCalendar ? 62 : 48) / 100 - 18 >= 154 && c.h - 32 >= 92;
  ...
  if (calendarSplit) {
    ...
    if (c.w < 430 && !expanded) {
      ...
      heroW = w * (wideLargeSingleCalendar ? 38 : 48) / 100;
