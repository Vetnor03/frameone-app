...
static void adaptiveCountdownMetric(const CountdownItem& item, char* out, size_t outSize) {
  char number[16], unit[16];
  formatDaysNumber(item.daysLeft, number, sizeof(number));
  formatDaysUnit(item.daysLeft, unit, sizeof(unit));
  snprintf(out, outSize, "%s %s", number, unit);
}
static int collectAdaptiveUpcoming(int heroIdx, const CountdownItem** out, int limit) {
  int n = 0;
  for (int i = 0; i < g_cache.count && n < limit; ++i)
    if (i != heroIdx && g_cache.items[i].used && !g_cache.items[i].isPast) out[n++] = &g_cache.items[i];
  return n;
}
static void adaptiveEffectiveDate(const CountdownItem& hero, char* out, size_t outSize) {
  CountdownAdaptivePolicy::effectiveDate(hero.displayDate, hero.targetDate, out, outSize);
}
static AdaptiveCountdownComposition adaptiveCountdownComposition(const Cell& c, const CountdownItem& hero, const CountdownItem** events, int eventCount) {
  char number[16], unit[16];
  formatDaysNumber(hero.daysLeft, number, sizeof(number));
  formatDaysUnit(hero.daysLeft, unit, sizeof(unit));
  CountdownAdaptivePolicy::Event policyEvents[MAX_EVENTS];
  char metrics[MAX_EVENTS][32];
  for (int i = 0; i < eventCount; ++i) {
    adaptiveCountdownMetric(*events[i], metrics[i], sizeof(metrics[i]));
    policyEvents[i] = {events[i]->title, metrics[i]};
  }
  const CountdownAdaptivePolicy::Input input{c.w, c.h, hero.title, number, unit, hero.displayDate, hero.targetDate, policyEvents, eventCount};
  const CountdownAdaptivePolicy::Result result = CountdownAdaptivePolicy::compose(input);
  AdaptiveCountdownComposition out;
  out.family = static_cast<AdaptiveCountdownComposition::Family>(result.family);
  out.showTitle = result.showTitle; out.showDate = result.showDate; out.showCalendar = result.showCalendar;
  out.upcomingRows = result.upcomingRows; out.overflow = result.overflow; out.splitPercent = result.splitPercent;
  return out;
}
