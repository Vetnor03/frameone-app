...
void render(const Cell& c) {
  ...
  if(policy.mode!=AiFollowAdaptivePolicy::UPDATES){
    ...
    if(secondary){char text[64];if(policy.mode==AiFollowAdaptivePolicy::ZERO_FOLLOW)strlcpy(text,g_cache->languageNo?"Folg temaer i appen":"Follow topics in the app",sizeof(text));else snprintf(text,sizeof(text),g_cache->languageNo?"Folger %u tema%s":"Following %u topic%s",g_cache->followingCount,g_cache->followingCount==1?"":"s");const Rect secondaryRect=horizontal?Rect{primaryRect.x+columnW+gap,start,columnW,20}:Rect{c.x+pad,start+36,c.w-pad*2,20};fit(text,fitted,sizeof(fitted),secondaryRect.w,&FreeSans9pt8b);drawInRect(secondaryRect,fitted,&FreeSans9pt8b,12,ALIGN_CENTER);}return;
  }
