...
void TempRefreshAudit::TEMP_REFRESH_AUDIT_record(const char* trigger, const String& sourceModules,
  ...
  doc["firmware_version"] = "v2.5.7"; doc["trigger"] = trigger;
  ...
  auditPrefs.putUChar("count", count < TEMP_REFRESH_AUDIT_MAX_RECORDS ? count + 1 : TEMP_REFRESH_AUDIT_MAX_RECORDS); auditPrefs.end();
#endif
}

void TempRefreshAudit::TEMP_REFRESH_AUDIT_flushPiggyback(const String& token) {
#if TEMP_REFRESH_AUDIT_ENABLED
