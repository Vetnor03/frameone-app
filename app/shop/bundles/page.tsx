  ...
  return <main className="shop-page h-screen overflow-y-auto bg-white text-[#141414]">
    <div className="shop-shell mx-auto max-w-[1720px] bg-white">
      ...
      <section className="mx-auto max-w-[1200px] px-6 py-8 md:py-12">
        <a href={`/shop?lang=${language}`} className="text-xs font-medium uppercase tracking-[.12em] text-black/60">← &nbsp; Back to home</a>
        ...
        <div className="mt-8 grid gap-x-5 gap-y-8 sm:grid-cols-2">
          {shopBundles.map((bundle) => <a key={bundle.id} href={`/shop/bundles/${bundle.id}?lang=${language}`} className="group overflow-hidden rounded-lg border border-black/10 bg-[#faf9f7] shadow-[0_10px_24px_rgba(0,0,0,.05)]">
            <div className="aspect-[16/10] overflow-hidden"><BundleArtwork bundle={bundle} /></div>
            <div className="p-5 sm:p-6">
              <div className="flex flex-wrap items-start justify-between gap-x-5 gap-y-3"><div className="min-w-0"><p className="text-[10px] font-medium uppercase tracking-[.15em] text-black/50">{bundle.eyebrow}</p><h2 className="mt-2 text-2xl font-medium">{bundle.name}</h2></div><span className="shrink-0 whitespace-nowrap rounded-full bg-[#dfe9d8] px-3 py-1 text-xs font-medium text-[#29421f]">Save {formatNok(bundleSavings(bundle), language)}</span></div>
