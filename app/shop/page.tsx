...
function pickCurrency(v?: string): Currency { return v === 'EUR' || v === 'USD' ? v : 'NOK' }
function formatPrice(valueNok: number, currency: Currency) {
  if (currency === 'NOK') return `${valueNok.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ')} NOK`
...
export default async function ShopPage({
  ...
  const frameCardsLocalized = frameCards.map((item) => ({
    ...item,
    price: formatPrice(item.priceNok, currency),
  }))
  ...
  const mattePrice = formatPrice(149, currency)
    ...
    >
      ...
      <div className="bg-[#0b0d10] text-[11px] text-white">
        <div className="mx-auto flex max-w-[1200px] items-center justify-center gap-3 px-6 py-2 tracking-[0.02em] sm:gap-5">
          <span>{`Free shipping over ${topShipping}`}</span>
        ...
        <div className="mx-auto max-w-[1200px]">
          <section className="relative py-10 md:min-h-[585px] md:py-0">
            <div className="relative z-10 mx-auto flex max-w-[26rem] flex-col px-6 py-3 md:-mt-6 md:min-h-[585px] md:max-w-none md:justify-center md:py-8 md:pl-14 md:pr-10">
              ...
              <a href="#waitlist" className="shop-button mt-9 w-fit rounded bg-black px-8 py-3 text-sm font-medium tracking-wide text-white md:mt-9">JOIN WAITLIST</a>
      ...
      <div className="bg-white">
        <div className="mx-auto max-w-[1200px] px-6 pb-14">
          <ShopReveal><section id="display" className="grid gap-8 border-b border-black/10 py-12 md:grid-cols-[1.05fr_0.95fr] md:items-center md:py-14">
          <div>
            <p className="text-sm font-medium uppercase tracking-[0.12em] text-black/50">RE:MIND Display</p>
            <h2 className="mt-3 text-[34px] font-semibold leading-[1.08] tracking-[-0.02em] sm:text-[46px]">The complete RE:MIND experience.</h2>
            <p className="mt-4 max-w-[36ch] text-lg leading-[1.45] text-black/65">A smart e-paper display for weather, reminders and calendar moments at home.</p>
          </div>
          <div className="rounded-lg border border-black/10 bg-[#faf9f7] p-6 shadow-[0_12px_26px_rgba(0,0,0,0.045)] md:p-8">
            <p className="text-sm uppercase tracking-[0.12em] text-black/50">Expected launch price</p>
            <p className="mt-2 text-[42px] font-medium leading-none tracking-[-0.03em]">{devicePrice}</p>
            <p className="mt-4 max-w-[34ch] text-sm leading-[1.55] text-black/65">Launching autumn 2026. Join the waitlist for launch updates and early-bird access.</p>
            <a href="#waitlist" className="shop-button mt-6 inline-flex rounded bg-black px-7 py-3 text-sm font-medium tracking-wide text-white">JOIN WAITLIST</a>
          </div>
        </section></ShopReveal>
          ...
          <div className="mb-6 flex flex-col items-start gap-2 sm:flex-row sm:items-end sm:justify-between sm:gap-4">
            ...
            <a className="shrink-0 text-sm uppercase tracking-[0.08em]" href="#waitlist">Reserve yours →</a>
          </div>
          ...
          <ShopReveal delayMs={110}><section id="bundles" className="grid gap-6 rounded-lg border border-black/10 bg-[#faf9f7] p-7 shadow-[0_12px_26px_rgba(0,0,0,0.045)] md:grid-cols-[1fr_auto] md:items-center md:p-9">
            <div>
              <p className="text-sm uppercase tracking-[0.12em] text-black/50">Bundle</p>
              <h2 className="mt-2 text-[30px] font-semibold leading-[1.08] tracking-[-0.01em]">Home Starter Pack</h2>
              <p className="mt-4 max-w-[58ch] text-sm leading-[1.6] text-black/65">Includes the full RE:MIND Display, one extra frame and a cleaning kit.</p>
            </div>
            <div className="md:text-right">
              <p className="text-sm uppercase tracking-[0.12em] text-black/50">Expected price</p>
              <p className="mt-2 text-[34px] font-medium tracking-[-0.02em]">{bundlePrice}</p>
              <a href="#waitlist" className="shop-button mt-4 inline-flex rounded bg-black px-7 py-3 text-sm font-medium tracking-wide text-white">Reserve yours</a>
            </div>
          </section></ShopReveal>

          <ShopReveal delayMs={130}><section id="waitlist" className="mt-12 rounded-lg border border-black/10 bg-white p-7 shadow-[0_12px_26px_rgba(0,0,0,0.045)] md:p-9">
            <div className="mb-6 max-w-[680px]">
              <p className="text-sm uppercase tracking-[0.12em] text-black/50">Waitlist</p>
              <h2 className="mt-2 text-[32px] font-semibold leading-[1.08] tracking-[-0.02em]">Be first in line for RE:MIND</h2>
              <p className="mt-4 text-sm leading-[1.6] text-black/65">Join the waitlist for launch updates and early-bird access. Planned launch is autumn 2026.</p>
            </div>
            <WaitlistForm productInterest="RE:MIND Display" />
          </section></ShopReveal>
        </div>
        </div>
      ...
      <footer id="about" className="border-t border-black/10 bg-white">
        ...
        <div className="mx-auto grid max-w-[1200px] gap-x-8 gap-y-9 px-6 py-10 text-sm sm:grid-cols-2 lg:grid-cols-[1.35fr_0.78fr_0.78fr_0.78fr_1.25fr]">
          ...
          <div><p className="mb-3 font-medium">STAY IN THE LOOP</p><p className="mb-3 max-w-[30ch] leading-[1.45] text-black/65">New frames, updates and ideas.</p><WaitlistForm compact productInterest="RE:MIND footer" /></div>
        </div>
