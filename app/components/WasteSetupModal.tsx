...
export default function WasteSetupModal({ language, onClose, onSaved }: { language: 'en' | 'no'; onClose: () => void; onSaved: (connected: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try { const response = await fetch(`/api/integrations/waste/search?q=${encodeURIComponent(query)}`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal }); const json = await response.json(); if (!response.ok) throw new Error(json.error); setResults(json.addresses || []) } catch (e) { if ((e as Error).name !== 'AbortError') setError(language === 'no' ? 'Adressesøket er midlertidig utilgjengelig. Prøv igjen.' : 'Address search is temporarily unavailable. Please retry.') }
    }, 350)
    return () => { window.clearTimeout(timer); controller.abort() }
  }, [query, selected, token, language])
  ...
  async function choose(address: Address) {
    ...
    catch (e) { const message = (e as Error).message; setError(message === 'Waste collection isn’t available for this address yet.' && language === 'no' ? 'Renovasjon er ikke tilgjengelig for denne adressen ennå.' : message || (language === 'no' ? 'Renovasjon er ikke tilgjengelig for denne adressen ennå.' : 'Waste collection isn’t available for this address yet.')) }
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 px-4 pb-4 sm:items-center"><div className="w-full max-w-sm rounded-3xl border border-[color:var(--bd-15)] bg-[color:var(--sheet-bg)] p-5 shadow-2xl">
    ...
    {error && <div className="mt-3 rounded-xl bg-[#d94b4b]/10 p-3 text-xs text-[#ff7a7a]">{error} <button onClick={() => selected && choose(selected)} className="ml-1 underline">{language === 'no' ? 'Prøv igjen' : 'Retry'}</button></div>}
