...
function assistantCopy(language: AppLanguage) {
  return language === 'no' ? {
    ...
    updates: 'Oppdateringer', instant: 'Radar', instantDescription: 'Radar følger ekstra godt med, slik at du holder deg oppdatert.', instantCounter: (count: number, max: number) => `${count} av ${max} Radar-følger`, instantFull: 'Alle Radar-plassene er i bruk.', instantUnavailable: 'Radar er tilgjengelig med Normal og Pro.', instantLimitReached: 'Alle Radar-plassene er i bruk.', instantOwnerOnly: 'Bare eieren kan slå Radar av eller på.', turnOnInstant: 'Slå på Radar', turnOffInstant: 'Slå av Radar',
    ...
    tasks: 'What RE:MIND is following', updates: 'Updates', instant: 'Radar', instantDescription: 'Radar keeps a closer eye on selected Watches, so you stay up to speed.', instantCounter: (count: number, max: number) => `${count} of ${max} Radar Watches`, instantFull: 'All Radar slots are in use.', instantUnavailable: 'Radar is available with Normal and Pro.', instantLimitReached: 'All Radar slots are in use.', instantOwnerOnly: 'Only the owner can turn Radar on or off.', turnOnInstant: 'Turn on Radar', turnOffInstant: 'Turn off Radar', emptyTasks: 'Ask RE:MIND to keep an eye on something, and it appears here.', emptyUpdates: 'New changes and updates appear here.',
