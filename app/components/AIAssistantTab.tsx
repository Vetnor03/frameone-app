...
function assistantCopy(language: AppLanguage) {
  return language === 'no' ? {
    ...
    updates: 'Oppdateringer', instant: 'Radar', instantDescription: 'Radar følger ekstra godt med, slik at du holder deg oppdatert.', instantCounter: (count: number, max: number) => `Radar på ${count} av ${max}`, instantFull: 'Alle Radar-plassene er i bruk.', instantUnavailable: 'Radar er ikke tilgjengelig med dette abonnementet.', instantLimitReached: 'Alle Radar-plassene er i bruk.', instantOwnerOnly: 'Bare eieren kan slå Radar av eller på.', turnOnInstant: 'Slå på Radar', turnOffInstant: 'Slå av Radar',
...
export default function AIAssistantTab({ language, activeDeviceId }: { language: AppLanguage; activeDeviceId: string | null }) {
  ...
  const accountSummary = entitlements
    ? `${planLabel} · ${c.limitCounter(ownedOngoingWatchCount, entitlements.max_ongoing_watches)}${entitlements.max_instant_watches > 0 ? ` · ${c.instantCounter(ownedInstantWatchCount, entitlements.max_instant_watches)}` : ''}`
    : c.loading
      ...
      {loading ? <p className="mt-4 text-sm text-[color:var(--fg-55)]">{c.loading}</p> : watches.length === 0 ? <p className="mt-4 rounded-3xl border border-dashed border-[color:var(--bd-20)] p-5 text-sm text-[color:var(--fg-55)]">{c.emptyTasks}</p> : <div className="mt-3 space-y-3">{watches.map((w) => {
        ...
        return <article key={w.id} onClick={() => setSelectedId(w.id)} className={`rounded-3xl border p-4 ${selected?.id === w.id ? 'border-[#2aa3ff]/70' : 'border-[color:var(--bd-15)]'} bg-[color:var(--card-bg)]/55`}>
          ...
          {canManageWatch && <div onClick={(e) => e.stopPropagation()} className="mt-4 flex items-center justify-between gap-3 rounded-2xl border border-[color:var(--bd-15)] p-3">
            ...
            <button type="button" role="switch" aria-checked={w.is_instant} aria-label={`${w.is_instant ? c.turnOffInstant : c.turnOnInstant}: ${w.title}`} disabled={busy || (!w.is_instant && cannotEnableInstant)} title={!w.is_instant && cannotEnableInstant ? (entitlements?.can_use_instant ? c.instantFull : c.instantUnavailable) : (w.is_instant ? c.turnOffInstant : c.turnOnInstant)} onClick={() => setWatchInstant(w, !w.is_instant)} className={`relative h-7 w-12 shrink-0 rounded-full transition ${w.is_instant ? 'bg-[#2aa3ff]' : 'bg-[color:var(--fg-25)]'} disabled:cursor-not-allowed disabled:opacity-45`}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white transition ${w.is_instant ? 'left-6' : 'left-1'}`} /></button>
          </div>}
