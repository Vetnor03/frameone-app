...
export const ASSISTANT_CAPABILITY_HANDLERS: Record<string, Handler> = {
  ...
  'settings.set_app_theme': { scope: 'user', destructive: false, missingQuestion: { theme: { en: 'Light or dark theme?', no: 'Lyst eller mørkt tema?' } }, validate: (args) => args.theme === 'light' || args.theme === 'dark' ? ok({ theme: args.theme }) : invalid('Theme must be light or dark.'), run: async (ctx, args) => { const theme = args.theme as 'dark' | 'light'; const { error } = await ctx.db.from('user_app_preferences').upsert({ user_id: ctx.user.id, app_theme: theme }, { onConflict: 'user_id' }); if (error) throw error; return { ...completed(ctx.language === 'no' ? 'Apptema oppdatert.' : 'App theme updated.'), appTheme: theme } } },
