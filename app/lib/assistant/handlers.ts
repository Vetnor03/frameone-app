...
export const ASSISTANT_CAPABILITY_HANDLERS: Record<string, Handler> = {
  ...
  'frame.set_layout': deviceSettingHandler('layout', BUILT_IN_LAYOUT_KEYS),
  ...
  'weather.read': { scope: 'device_member', destructive: false, missingQuestion: {}, validate: () => ok({}), run: async (ctx) => {
    ...
    const response = await ctx.weatherDetails(new Request(`http://frame.local/api/weather/details?lat=${lat}&lon=${lon}`)); const payload = await response.json() as any
  ...
  'surf.read': { scope: 'device_member', destructive: false, missingQuestion: {}, validate: (args) => ok(typeof args.spot === 'string' ? { spot: args.spot } : {}), run: async (ctx, args) => {
    ...
    const rating = Number(payload.rating ?? payload.score); return completed(ctx.language === 'no' ? `${payload.spot ?? spot}: ${rating} av 6.` : `${payload.spot ?? spot}: ${rating} out of 6.`)
  } },
...
// Inventory entries that do not yet need a write/read adapter are real
