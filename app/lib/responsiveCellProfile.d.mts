...
export function studioRenderStrategy(module:string,colSpan:number,rowSpan:number,width:number,height:number):{path:'legacy'|'responsive'|'reminders-responsive'|'weather-responsive'|'countdown-responsive'|'date-responsive'|'surf-responsive'|'soccer-responsive'|'stocks-responsive'|'groceries-responsive';legacyVariant:'SMALL'|'MEDIUM'|'LARGE'|'XL'|null;profile:ResponsiveCellProfile}
