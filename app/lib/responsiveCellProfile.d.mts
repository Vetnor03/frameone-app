...
export function studioRenderStrategy(module:string,colSpan:number,rowSpan:number,width:number,height:number):{path:'legacy'|'responsive'|'reminders-responsive'|'weather-responsive';legacyVariant:'SMALL'|'MEDIUM'|'LARGE'|'XL'|null;profile:ResponsiveCellProfile}
