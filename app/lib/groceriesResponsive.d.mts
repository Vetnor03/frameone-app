...
export function fitGroceriesText(value:string,width:number,measure:(text:string,fontSize:number)=>number,options?:{fontSize?:number;ellipsis?:string}):{text:string;truncated:boolean}
export function fitRunningLowText(item:RunningLowItem,width:number,measure:(text:string,fontSize:number)=>number,options?:{fontSize?:number}):{text:string;labelShown:boolean;nameTruncated:boolean}
export function fitMealIdeaText(idea:MealIdea,width:number,measure:(text:string,fontSize:number)=>number,options?:{fontSize?:number}):{text:string;missingShown:number;titleTruncated:boolean}
