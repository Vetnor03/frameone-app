...
export type CanonicalGroceryMergeCandidate = { isChecked: boolean; category: string }

/** Mirrors the priority encoded by add_grocery_items_canonical for testable callers. */
export function canonicalGroceryMergePriority(candidate: CanonicalGroceryMergeCandidate, requestedCategory: string) {
  if (!candidate.isChecked && candidate.category === requestedCategory) return 1
  if (!candidate.isChecked) return 2
  if (candidate.category === requestedCategory) return 3
  return null
}
