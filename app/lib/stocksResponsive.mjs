...
export function stocksLayout(profile,composition){
  ...
  else if(composition.family==='expanded'){const threeByFour=profile.colSpan===3&&profile.rowSpan===4,chartH=Math.min(h*.40,h-245);summaryGroupRect=rect(pad,pad,w-pad*2,threeByFour?84:100);detailsRect=rect(pad,summaryGroupRect.y+summaryGroupRect.height+8,w-pad*2,threeByFour?64:78);if(composition.showSelector)rangeSelectorRect=rect(pad,detailsRect.y+detailsRect.height+5,w-pad*2,28);const cy=(rangeSelectorRect?rangeSelectorRect.y+rangeSelectorRect.height:detailsRect.y+detailsRect.height)+7;chartRect=rect(pad,cy,w-pad*2,h-pad-cy)}
