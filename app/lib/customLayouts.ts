import {
  ...
  supportsPhysicalCustomCell as supportsCell,
...
const strictValidateCustomGeometry = validate as (cells:unknown, options?:{requirePhysical?:boolean;requireModules?:boolean})=>{valid:boolean;errors:string[];unsupportedSlots:number[]}

/**
 * The Frame screen owns geometry and module assignments separately. A geometry-only
 * precheck therefore validates structure here; the merged payload below performs the
 * authoritative module-aware physical check. API callers that provide modules keep
 * the strict physical gate.
 */
export const validateCustomGeometry = ((cells:unknown, options:{requirePhysical?:boolean;requireModules?:boolean}={})=>{
  const hasInlineModules = Array.isArray(cells) && cells.some(cell => cell && typeof cell === 'object' && 'module' in cell)
  if (options.requirePhysical && !options.requireModules && !hasInlineModules) {
    return strictValidateCustomGeometry(cells, { ...options, requirePhysical: false })
  }
  return strictValidateCustomGeometry(cells, options)
}) as (cells:unknown, options?:{requirePhysical?:boolean;requireModules?:boolean})=>{valid:boolean;errors:string[];unsupportedSlots:number[]}
