...
export function createHimProvider(fetcher: Fetch = fetch): WasteProvider {
  ...
  return { family: 'him', resolveAddress: async address => address,
    ...
    normalizeCollections(raw) {
      ...
      for (const section of r.html.matchAll(/<div\b[^>]*class=["'][^"']*tommekalender__section[^"']*tommekalender__month[^"']*["'][^>]*>([\s\S]*?)<\/div>\s*(?=<div\b[^>]*class=["'][^"']*tommekalender__section|$)/gi)) {
        const heading = section[1].match(/<h2\b[^>]*>\s*([^<]+)\s*<\/h2>/i)?.[1].toLocaleLowerCase('nb-NO') || ''
        const context = heading.match(/([a-zæøå]+)\s+((?:19|20)\d{2})/i), month = context && months[context[1]], year = context?.[2]
        if (!month || !year) continue
        for (const cell of section[1].matchAll(/<(?:td|div)\b[^>]*class=["'][^"']*tommekalender__calendartable__date[^"']*["'][^>]*>([\s\S]*?)<\/(?:td|div)>/gi)) {
          const day = cell[1].replace(/<[^>]+>/g, ' ').match(/\b([0-3]?\d)\b/)?.[1]; if (!day) continue
...
export function createOsloProvider(fetcher: Fetch = fetch): WasteProvider {
  ...
  return { family: 'oslo', resolveAddress: async address => address,
    ...
    normalizeCollections(raw) { const root = record(raw), results = root.result; if (!Array.isArray(results)) throw new WasteProviderError('invalid_response', 'Oslo returned an invalid response.'); const services = results.flatMap((result: unknown) => { const points = record(result).HentePunkts; return Array.isArray(points) ? points.flatMap((point: unknown) => Array.isArray(record(point).Tjenester) ? record(point).Tjenester : []) : [] }); const rows = services.flatMap((entry: unknown) => { const e = record(entry), fraction = record(e.Fraksjon), rawType = string(fraction.Tekst), typeId = string(fraction.Id), eventId = string(e.Id), start = string(e.TommeDato).slice(0, 10), factor = Number(record(e.Hyppighet).Faktor); if (!ymd.test(start) || !Number.isFinite(factor) || factor <= 0) return [makeCollection({ date: start, rawType, providerTypeId: typeId || undefined, providerEventId: eventId || undefined, semanticSource: 'json_field', raw: entry })]; const dates: string[] = []; const date = new Date(`${start}T12:00:00Z`); for (let weeks = 0; weeks < 53; weeks += factor) { const occurrence = new Date(date); occurrence.setUTCDate(date.getUTCDate() + weeks * 7); dates.push(occurrence.toISOString().slice(0, 10)) } return dates.map(value => makeCollection({ date: value, rawType, providerTypeId: typeId || undefined, providerEventId: eventId ? `${eventId}:${value}` : undefined, semanticSource: 'json_field', raw: entry })) }); return finish(rows, 'Oslo') }
  }
}
