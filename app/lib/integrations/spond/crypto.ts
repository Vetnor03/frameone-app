...
function getKey() {
  const raw = process.env.SPOND_CREDENTIALS_KEY || ''
  if (!raw) throw new Error('Missing SPOND_CREDENTIALS_KEY')

  const key = Buffer.from(raw, 'base64')
  if (key.length !== 32) throw new Error('SPOND_CREDENTIALS_KEY must be a base64-encoded 32-byte key')
  return key
}
