...
export type ReminderComposition={available:boolean;direction:'horizontal'|'vertical'|'split';family:'shallow-horizontal'|'vertical-list'|'split-sections';showHeading:boolean;showTime:boolean;showTomorrow:boolean;todayItems:number;tomorrowItems:number;todayOverflow:number;tomorrowOverflow:number;maxItems:number;overflow:number;selectedFont:'B12'|'B9'|null;splitRatio:number|null;readabilityScore:number}
export type ReminderDensity={name:'normal'|'dense';font:'B12'|'B9';fontSize:number;rowHeight:number;rowGap:number;timeWidth:number}
...
export function estimateReminderTextWidth(value:string,font?:'B12'|'B9'):number
