...
export default function HomePage() {
  ...
  function restoreManualUpdateState(deviceId: string | null) {
  ...
  useEffect(() => {
    ...
    restoreManualUpdateState(activeDeviceId)
  ...
  async function logout() {
    try {
      ...
      console.warn('[notifications:logout-cleanup-failed]', { message: errorMessage(error) })
    }
...
function ConnectAppsScreen({
  ...
  async function fetchSpondStatus() {
    ...
    if (typeof json?.setup_error?.message === 'string' && json.setup_error.message) {
      ...
      setIntegrationSetupErrors((current) => ({ ...current, spond: undefined }))
    }
  }
  ...
  async function connectTeams() {
    ...
    const teamsSetupError = integrationSetupErrors.teams
    if (teamsSetupError) {
      setStatusTone('error')
      setStatus(teamsSetupError)
      return
    }
...
function MirrorAiAssistantLayout({ header, children, className = '', contentClassName = '' }: { header: string; children: React.ReactNode; className?: string; contentClassName?: string }) {
  return (
    <div className={`relative flex h-full w-full flex-col items-center overflow-hidden text-center leading-none ${className}`}>
      <div className={MIRROR_ASSISTANT_HEADER_ROW_CLASS}>
...
function MirrorAiAssistantCard({ detail, language, mutedColor, borderColor, maxItems, variant }: { detail: MirrorModuleDetail; language: AppLanguage; mutedColor: string; borderColor: string; maxItems: number; variant: 'small' | 'medium' | 'large' | 'xl' }) {
  ...
  if (items.length <= 0) {
    return (
      <MirrorAiAssistantLayout
        header={header}
        className={variant === 'small' ? MIRROR_ASSISTANT_SMALL_SHELL_CLASS : MIRROR_ASSISTANT_SHELL_CLASS}
        contentClassName={variant === 'small' ? MIRROR_ASSISTANT_SMALL_BODY_CLASS : MIRROR_ASSISTANT_BODY_CLASS}
      >
        <div className={`max-w-full leading-[1.18] ${MIRROR_SECONDARY_TEXT_CLASS}`} style={{ color: mutedColor }}>{mirrorAiAssistantEmptyStatus(detail, language)}</div>
      </MirrorAiAssistantLayout>
    )
  }
  ...
  return (
    <MirrorAiAssistantLayout
      header={header}
      className={MIRROR_ASSISTANT_SHELL_CLASS}
...
async function unregisterCurrentPushSubscription() {
  ...
  if (!response.ok) throw new Error('push_subscription_unregister_failed')
}
...
function groceryCheckedExpiresAtMs(item: Pick<GroceryItem, 'isChecked' | 'checkedAt'>) {
...
function groceryCheckedItemIsExpired(item: Pick<GroceryItem, 'isChecked' | 'checkedAt'>, nowMs: number) {
...
function groceryIsVisible(item: Pick<GroceryItem, 'isChecked' | 'checkedAt'>, nowMs: number) {
...
function GroceriesModuleSettingsTab({
  ...
  async function addRecipeItems(recipeItems: Array<GroceryRecipeItem & { category: string }>) {
    if (!activeDeviceId) return
    const working = items.filter((item) => !isDinnerVirtualId(item.id) && groceryIsVisible(item, Date.now())).map((item) => ({ ...item }))
    const { data: authData } = await supabase.auth.getUser()
    let firstError: string | null = null
    for (const incoming of recipeItems) {
      const decision = recipeMergeDecision(working, incoming)
      if (decision.type === 'merge') {
        const existing = working[decision.index]
        const { error } = await supabase.from('grocery_items').update({ quantity: decision.quantity, amount: decision.amount, unit: decision.unit, is_checked: false, checked_at: null }).eq('id', existing.id)
        if (error) { firstError ??= error.message; continue }
        working[decision.index] = { ...existing, quantity: decision.quantity, amount: decision.amount, unit: decision.unit, isChecked: false, checkedAt: null }
      } else {
        const { data, error } = await supabase.from('grocery_items').insert({ device_id: activeDeviceId, created_by: authData.user?.id ?? null, name: incoming.name, quantity: incoming.quantity, amount: incoming.amount, unit: incoming.unit, category: asGroceryCategory(incoming.category), is_checked: false, checked_at: null }).select('id, name, quantity, amount, unit, category, is_checked, checked_at, updated_at').single()
        if (error) { firstError ??= error.message; continue }
        const inserted = groceryItemFromRow(data)
        if (inserted) working.push(inserted)
      }
      await rememberHistoryItem(incoming.name, asGroceryCategory(incoming.category))
    }
    await loadGroceries({ silent: true, preserveScroll: true })
    await loadHistory()
            ...
            {group.items.map((item) => (
              ...
              >
                ...
                {item.amount != null && item.unit ? (
                  <div className="shrink-0 text-sm text-[color:var(--fg-55)] [font-variant-numeric:tabular-nums]">{item.amount} {item.unit}</div>
    ...
    {recipeOpen && activeDeviceId ? (
      <RecipeSheet
        ...
        onAdd={async (ingredients) => {
          await addRecipeItems(ingredients)
        }}
      />
    ) : null}
    </>
...
function RecipeSheet({ language, deviceId, onClose, onAdd }: { language: AppLanguage; deviceId: string; onClose: () => void; onAdd: (items: Array<GroceryRecipeItem & { category: string }>) => Promise<void> }) {
  ...
  async function importUrl() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error || (language === 'no' ? 'Importeringen mislyktes.' : 'Import failed.'))
  ...
  async function deleteRecipe() {
    if (!savedRecipeId || !window.confirm(language === 'no' ? 'Slette denne oppskriften?' : 'Delete this recipe?')) return
    setPendingAction('delete'); setSuccess(null); setError('')
    try {
      ...
      setError(deleteError instanceof Error ? `${language === 'no' ? 'Kunne ikke slette oppskriften' : 'Could not delete recipe'}: ${deleteError.message}` : (language === 'no' ? 'Kunne ikke slette oppskriften.' : 'Could not delete recipe.'))
    } finally { setPendingAction(null) }
  }
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-[color:var(--overlay-55)]">
    <div className="w-full max-w-[420px] max-h-[92vh] overflow-y-auto rounded-t-3xl bg-[color:var(--sheet-bg)] border-t border-[color:var(--bd-10)] p-5">
      ...
      {mode === 'start' ? <div className="mt-5 grid gap-3">
        ...
        <button onClick={() => { void loadSaved() }} className="h-10 text-xs text-[color:var(--fg-55)]">{language === 'no' ? 'LAGREDE OPPSKRIFTER' : 'SAVED RECIPES'}</button>
      </div> : null}
      ...
      {mode === 'preview' && draft ? <div className="mt-4">
        ...
        <div className="mt-4 divide-y divide-[color:var(--bd-10)] rounded-2xl border border-[color:var(--bd-10)]">{ingredients.map((item, index) => {
          ...
          return <div key={index} className={`flex items-center gap-2 p-3 transition ${item.selected ? '' : 'bg-[color:var(--panel-05)] text-[color:var(--fg-45)]'}`}>
            <input aria-label={`${item.name}: ${language === 'no' ? 'må kjøpes' : 'needs to be bought'}`} type="checkbox" checked={item.selected} onChange={() => setIngredients((all) => all.map((x,i) => i === index ? { ...x, selected: !x.selected } : x))}/>
            <div className="min-w-0 flex-1">
              {savedRecipeId ? <div>{item.name}</div> : <input value={item.name} onChange={(e) => setIngredients((all) => all.map((x,i) => i === index ? { ...x, name: e.target.value } : x))} className="w-full bg-transparent outline-none"/>}
              {!item.selected ? <div className="mt-0.5 text-[10px] uppercase tracking-wider text-[color:var(--fg-45)]">{language === 'no' ? 'Har hjemme' : 'Already have'}</div> : null}
            </div>
            {savedRecipeId ? <div className="shrink-0 text-right text-[color:var(--fg-55)]">{qty == null ? '' : qty}{qty != null && item.unit ? ' ' : ''}{item.unit || ''}</div> : <>
              <input aria-label="Quantity" type="number" step="any" value={qty ?? ''} onChange={(e) => { const scaled = e.target.value === '' ? null : Number(e.target.value); const base = scaled == null || !draft.servings || !servings ? scaled : scaled * draft.servings / servings; setIngredients((all) => all.map((x,i) => i === index ? { ...x, quantity: base } : x)) }} className="w-14 bg-transparent text-right outline-none"/>
              <input aria-label="Unit" value={item.unit || ''} onChange={(e) => setIngredients((all) => all.map((x,i) => i === index ? { ...x, unit: e.target.value || null } : x))} className="w-12 bg-transparent text-[color:var(--fg-55)] outline-none"/>
            </>}
          </div>
        })}</div>
        <div className={`mt-4 ${savedRecipeId ? '' : 'flex gap-2'}`}>
          {!savedRecipeId ? <button disabled={busy} onClick={saveRecipe} className="h-11 flex-1 rounded-2xl border border-[color:var(--bd-15)] text-xs disabled:opacity-40">{pendingAction === 'save' ? (language === 'no' ? 'LAGRER OPPSKRIFT…' : 'SAVING RECIPE…') : (language === 'no' ? 'LAGRE OPPSKRIFT' : 'SAVE RECIPE')}</button> : null}
          <button disabled={!selected.length || busy} onClick={addSelectedIngredients} className="h-11 w-full flex-1 rounded-2xl border border-[#2aa3ff] text-[#2aa3ff] text-xs disabled:opacity-40">{pendingAction === 'add' ? (language === 'no' ? 'LEGGER TIL I HANDLELISTEN…' : 'ADDING TO GROCERIES…') : (language === 'no' ? `LEGG ${selected.length} I HANDLELISTEN` : `ADD ${selected.length} TO GROCERIES`)}</button>
        </div>
        {savedRecipeId ? <button disabled={busy} onClick={deleteRecipe} className="mt-4 w-full py-2 text-[10px] tracking-widest text-red-400/80 transition hover:text-red-400 disabled:opacity-40">{pendingAction === 'delete' ? (language === 'no' ? 'SLETTER OPPSKRIFT…' : 'DELETING RECIPE…') : (language === 'no' ? 'SLETT OPPSKRIFT' : 'DELETE RECIPE')}</button> : null}
      </div> : null}
...
function Switch({ checked, onChange }: { checked: boolean; onChange: (next: boolean) => void }) {
  return (
    ...
    >
      <span
        className={`block h-6 w-6 rounded-full bg-[color:var(--switch-thumb)] transition-transform ${checked ? 'translate-x-5' : 'translate-x-0'}`}
        style={{ boxShadow: '0 6px 18px var(--overlay-55)' }}
      />
    </button>
  )
}
...
function CustomSurfSpotWizard({ language, onClose, onSaved, editingSpot = null, onDeleted }: CustomSurfSpotWizardProps) {
  ...
  return <div className="fixed inset-0 z-[60] bg-[color:var(--app-bg)] text-[color:var(--fg)] flex flex-col">
    <div data-ui="sheet" className="absolute left-4 right-4 top-4 z-20 rounded-2xl p-4 backdrop-blur-sm">
      ...
      {step === 1 ? <><input data-ui="input" value={name} onChange={(e) => setName(e.target.value)} placeholder={language === 'no' ? 'Spotnavn' : 'Spot name'} className="mt-2 w-full h-11 rounded-xl px-3 outline-none" />
    ...
    <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 px-3 pb-[max(20px,calc(env(safe-area-inset-bottom)+10px))]">
      <div className="pointer-events-auto grid grid-cols-2 gap-3">
        {step === 1 ? <button data-ui="button" className="h-12 rounded-xl" onClick={onClose}>{language === 'no' ? 'Lukk' : 'Close'}</button> : <button data-ui="button" className="h-12 rounded-xl" onClick={() => setStep((step - 1) as 1 | 2 | 3)}>{language === 'no' ? 'Tilbake' : 'Back'}</button>}
        {step < 3 ? (
          <button
          ...
          <button data-ui="button-primary" disabled={saving || !name.trim()} className="h-12 rounded-xl" onClick={save}>{saving ? 'Saving…' : (language === 'no' ? 'Lagre endringer' : 'Save changes')}</button>
        )}
      </div>
        ...
        <>
          ...
          <div className="mt-4 divide-y divide-[color:var(--bd-10)] border-t border-[color:var(--bd-10)]">
            ...
            <div className="grid grid-cols-[1fr_auto] items-center gap-4 py-3">
              ...
              <div className="w-20 text-right text-sm font-medium text-[color:var(--fg-75)]">{weatherDetailUvLabel(language, data.currentUvIndex)}</div>
            </div>
