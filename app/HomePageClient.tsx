...
function CustomSurfSpotEditorSheet({ language, onClose, onSave }: { language: AppLanguage; onClose: () => void; onSave: (spot: CustomSurfSpot) => void | Promise<void> }) {
  ...
  const isNo = language === 'no'
  const stepSize = zoom >= 14 ? 0.0025 : zoom >= 12 ? 0.006 : 0.015

  function nudgeMap(dLat: number, dLon: number) {
    setMapLat((v) => Math.max(-85, Math.min(85, v + dLat)))
    setMapLon((v) => {
      const next = v + dLon
  ...
  return <div className="fixed inset-0 z-[60] flex items-end justify-center bg-[color:var(--overlay-55)]">
    <div className="w-full max-w-[420px] max-h-[92vh] overflow-y-auto rounded-t-3xl bg-[color:var(--sheet-bg)] border-t border-[color:var(--bd-10)] px-5 pt-5 pb-8">
      ...
      <div className="mt-2 text-xs text-[color:var(--fg-50)]">{isNo ? `STEG ${step} AV 3` : `STEP ${step} OF 3`}</div>

      {step === 1 && <>
        <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder={isNo ? 'Navn på spot' : 'Spot name'} className="mt-3 w-full h-11 rounded-2xl border border-[color:var(--bd-10)] bg-[color:var(--panel-05)] px-4" />
        <div className="mt-3 text-xs text-[color:var(--fg-60)]">{isNo ? 'Finn bølgebrekket og sentrer blå prikk over spoten.' : 'Locate the break and center the blue dot over it.'}</div>
        <MapCenterPicker lat={mapLat} lon={mapLon} zoom={zoom} onZoom={setZoom} onNudge={nudgeMap} />
        ...
        <div className="mt-3 text-xs text-[color:var(--fg-60)]">{isNo ? 'Velg bølgeretning og hovedretning.' : 'Choose swell working sector and main direction.'}</div>
      ...
      {step === 2 && <>
        <div className="mt-3 text-xs text-[color:var(--fg-60)]">{isNo ? 'Velg vindretning som fungerer, og foretrukket hovedretning.' : 'Choose wind working sector and preferred main wind direction.'}</div>
      ...
      {step === 3 && <>
        <div className="mt-3 text-xs text-[color:var(--fg-60)]">{isNo ? 'Sentrer blå prikk over nærmeste parkering.' : 'Center the blue dot over closest parking.'}</div>
        <MapCenterPicker lat={mapLat} lon={mapLon} zoom={zoom} onZoom={setZoom} onNudge={nudgeMap} />
        ...
        <div className="mt-2 text-xs text-[color:var(--fg-50)]">Break: {breakLat.toFixed(5)}, {breakLon.toFixed(5)} · Parking: {parkingLat.toFixed(5)}, {parkingLon.toFixed(5)}</div>
      </>}
      ...
      <div className="mt-2 text-xs text-[color:var(--fg-50)]">Center: {center}</div>
    </div>
  </div>
}
...
function MapCenterPicker({ lat, lon, zoom, onZoom, onNudge }: { lat: number; lon: number; zoom: number; onZoom: (z: number) => void; onNudge: (dLat: number, dLon: number) => void }) {
  ...
  return <div className="mt-2">
    ...
    <div className="mt-2 grid grid-cols-3 gap-2">
      <button className="h-9 rounded-xl border" onClick={() => onNudge(d, 0)}>↑</button>
      <button className="h-9 rounded-xl border" onClick={() => onZoom(Math.min(16, zoom + 1))}>＋</button>
      <button className="h-9 rounded-xl border" onClick={() => onNudge(0, d)}>→</button>
      <button className="h-9 rounded-xl border" onClick={() => onNudge(0, -d)}>←</button>
      <button className="h-9 rounded-xl border" onClick={() => onZoom(Math.max(9, zoom - 1))}>－</button>
      <button className="h-9 rounded-xl border" onClick={() => onNudge(-d, 0)}>↓</button>
    </div>
  </div>
}
