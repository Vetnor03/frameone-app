...
type PhysicalFrameBatteryStatus = {
  percent: number | null
  isCharging: boolean
}
...
export default function HomePage() {
  ...
  const [physicalFrameBattery, setPhysicalFrameBattery] = useState<PhysicalFrameBatteryStatus>({
    percent: null,
    isCharging: false,
  })
  ...
  async function loadDeviceStatus(deviceId: string): Promise<string | null> {
    try {
      ...
      if (!resp.ok) {
        ...
        setPhysicalFrameBattery({ percent: null, isCharging: false })
        return null
      }
      ...
      setPhysicalFrameBattery({
        percent: normalizeBatteryPercent(data?.battery_percent),
        isCharging: normalizeBoolean(data?.is_usb_present) === true || normalizeBoolean(data?.is_charging) === true,
      })
...
function LandscapeBatteryIndicator({
  battery,
  mutedColor,
  textColor,
}: {
  battery: PhysicalFrameBatteryStatus
  mutedColor: string
  textColor: string
}) {
  const percentLabel = battery.percent == null ? '--%' : `${battery.percent}%`
  const fillPercent = battery.percent == null ? 0 : Math.max(0, Math.min(100, battery.percent))

  return (
    <div className="flex items-center gap-2 text-[11px] font-semibold tracking-[0.16em]" style={{ color: textColor }}>
      <span className="tabular-nums leading-none" style={{ color: mutedColor }}>{percentLabel}</span>
      <svg
        aria-hidden
        viewBox="0 0 28 14"
        className="h-[14px] w-[28px] shrink-0"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <rect x="1" y="1.5" width="23" height="11" rx="2.5" stroke="currentColor" strokeWidth="1.4" opacity="0.9" />
        <rect x="24.7" y="5" width="2.3" height="4" rx="1" fill="currentColor" opacity="0.9" />
        <rect
          x="3.2"
          y="3.8"
          width={Math.max(1.5, (18.6 * fillPercent) / 100)}
          height="6.4"
          rx="1.2"
          fill="currentColor"
          opacity={battery.percent == null ? 0.18 : 0.85}
        />
      </svg>
      {battery.isCharging && (
        <svg
          aria-label="Charging"
          viewBox="0 0 10 16"
          className="h-4 w-3 shrink-0 text-white"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path d="M6.1 0.8 1.8 8.1h3L3.8 15.2l4.4-8.1h-3Z" fill="currentColor" />
        </svg>
      )}
    </div>
  )
    ...
    >
      ...
      >
        ...
        <div className="absolute right-4 top-3 z-10">
          <LandscapeBatteryIndicator battery={battery} mutedColor={mutedColor} textColor={textColor} />
        </div>
