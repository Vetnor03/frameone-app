...
function surfDebugConditionLog(args: {
  ...
  console.info(args.cardMode === 'current' ? '[surf-score:current-card]' : '[surf-score:condition]', {
    ...
    windDirectionSemantic: scoredTables?.wind_dir?.windDirectionSemantic ?? 'from',
    windDirectionCompass: scoredTables?.wind_dir?.windDirectionCompass ?? degToCompass8(args.condition.marine.wind_direction_deg_from),
    windDirectionScoreReason: scoredTables?.wind_dir?.windDirectionScoreReason ?? null,
