...
Deno.serve(async (req) => {
  ...
  if (Deno.env.get('RADAR_SOURCE_PROBE_MODE') !== 'shadow') return Response.json({ok:true,mode:'off',claimed:0})
  ...
  const results=[]; for(const job of jobs||[]) results.push(await processJob(db,job)); return Response.json({ok:true,mode:'shadow',claimed:jobs?.length||0,results})
})
...
async function recordSuccess(db:any,job:any,source:any,r:any) { const now=new Date().toISOString(); const changed=r.outcome==='changed'; await db.from('monitoring_source_probes').insert({source_id:source.id,watch_id:source.watch_id,owner_user_id:source.owner_user_id,outcome:r.outcome,http_status:r.status,change_detected:changed,previous_fingerprint:source.content_fingerprint,new_fingerprint:r.fingerprint,etag:r.etag,last_modified:r.lastModified,content_type:r.type,bytes_read:r.bytes,duration_ms:r.duration,signal_details:{source_type:r.sourceType}}); await db.from('monitoring_watch_sources').update({etag:r.etag,last_modified:r.lastModified,content_type:r.type,content_length:r.bytes,content_fingerprint:r.fingerprint,source_type:r.sourceType,last_checked_at:now,last_changed_at:changed?now:source.last_changed_at,next_probe_at:new Date(Date.now()+15*60000).toISOString(),consecutive_errors:0,disabled_reason:null}).eq('id',source.id); await db.from('monitoring_source_probe_queue').update({completed_at:now,last_error:null}).eq('id',job.id); return {job_id:job.id,ok:true,outcome:r.outcome} }
