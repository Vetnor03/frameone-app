...
async function processJob(db:any,job:any,mode:string) {
  ...
  try {
    ...
    if(status===304) return await recordSuccess(db,job,source,{outcome:'not_modified',status,bytes,type,etag,lastModified,fingerprint:source.content_fingerprint,sourceType:source.source_type,duration:Date.now()-started},mode)
    ...
    return await recordSuccess(db,job,source,{outcome:source.content_fingerprint&&source.content_fingerprint!==fingerprint?'changed':'unchanged',status,bytes,type,etag,lastModified,fingerprint,sourceType:normalized.sourceType,duration:Date.now()-started},mode)
  } catch(error) {
...
async function recordSuccess(db:any,job:any,source:any,r:any,mode:string) {
  ...
  if(changed&&mode==='guarded'&&probe?.id){
    ...
    if(allow){
      const rawHours=Number(Deno.env.get('RADAR_STRONG_SOURCE_DISCOVERY_HOURS')||12); const hours=Number.isFinite(rawHours)?rawHours:12
      const {data:decision,error:decisionError}=await db.rpc('get_guarded_watch_decision',{p_watch_id:source.watch_id,p_owner_allowlisted:true,p_discovery_hours:hours}).maybeSingle()
      if(!decisionError&&decision&&typeof decision.can_gate==='boolean'&&decision.can_gate) await db.rpc('record_guarded_source_change',{p_watch_id:source.watch_id,p_source_id:source.id,p_probe_id:probe.id,p_reason:'meaningful_fingerprint_change'})
      else if(decisionError||!decision||typeof decision.can_gate!=='boolean'||['missing_sources','sources_missing_stale_or_failing','eligibility_uncertain'].includes(String(decision?.reason))) await db.rpc('enqueue_monitoring_safety_fallback',{p_watch_id:source.watch_id,p_source_id:source.id,p_probe_id:probe.id,p_reason:decisionError?'guarded_decision_rpc_error':String(decision?.reason||'guarded_decision_uncertain')})
    }
  }
  await db.from('monitoring_source_probe_queue').update({completed_at:now,last_error:null}).eq('id',job.id); return {job_id:job.id,ok:true,outcome}
}
