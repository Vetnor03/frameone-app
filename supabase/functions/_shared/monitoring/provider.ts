...
function detectSubjectEntities(text: string, category: string) {
  const entities: string[] = []
  if (/\bviking\b/i.test(text) && category === 'sports:soccer') entities.push('viking fk')
  for (const q of text.matchAll(/\b(?:team|club|company|artist|band|laget|klubben|selskapet)\s+([A-ZÆØÅ][\p{L}0-9&.-]{1,40})/gu)) entities.push(q[1])
  return canonicalArray(entities, 6)
}
...
export function canonicalizeWatchIntent(watch: Record<string, unknown>): CanonicalWatchIntent | null {
  ...
  const source = [watch.original_request, watch.normalized_goal, queryText].join(' ')
  const category = classifyTopic(source)
  const contentType = classifyContentType(source)
  const location = detectLocation(source)
  if (category === 'general' && !location && !queryText) return null
  return {
    schema_version: 2,
    topic_category: category,
    subject_entities: detectSubjectEntities(source, category),
    geographic_location: location,
    radius_constraint: detectRadius(source),
    content_type: contentType,
    important_filters: canonicalArray([...detectImportantFilters(source), ...detectImportantFilters(filterText)], 8),
    time_horizon: detectTimeHorizon(source),
    search_scope: location ? 'local_public_web' : 'public_web',
  }
}
