-- Learn household-specific grocery availability windows for meal idea matching.
-- Example: if milk is usually marked out 6 days after purchase, recipes stop
-- treating milk as likely available after that learned window passes.

alter table public.grocery_item_history
  add column if not exists purchase_count integer not null default 0,
  add column if not exists last_purchased_at timestamptz,
  add column if not exists last_marked_out_at timestamptz,
  add column if not exists average_days_available numeric(8,2);

update public.grocery_item_history
set
  purchase_count = greatest(purchase_count, usage_count),
  last_purchased_at = coalesce(last_purchased_at, last_used_at)
where last_purchased_at is null
   or purchase_count < usage_count;

create index if not exists grocery_item_history_device_purchase_idx
