...
create or replace function public.set_user_surf_spots_updated_at()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists trg_user_surf_spots_updated_at on public.user_surf_spots;
create trigger trg_user_surf_spots_updated_at
before update on public.user_surf_spots
for each row
execute function public.set_user_surf_spots_updated_at();
