...
declare uid uuid := auth.uid(); entry jsonb; clean_name text; clean_key text; qty integer; cat text; effective_cat text; existing grocery_items%rowtype; history grocery_item_history%rowtype; ids uuid[] := '{}'; prior jsonb; claimed boolean;
begin
  ...
  for entry in select * from jsonb_array_elements(p_items) loop
    ...
    select * into existing from grocery_items
      where device_id=p_device_id
        and lower(regexp_replace(btrim(name),'\s+',' ','g'))=clean_key
        and amount is null and unit is null
        and (not is_checked or (checked_at > now()-interval '10 minutes' and category=cat))
      order by case
        when not is_checked and category=cat then 1
        when not is_checked then 2
        when is_checked and category=cat then 3
        else 4
      end, updated_at desc
      limit 1 for update;
    if found then
    ...
    if found then
      update grocery_item_history set name=clean_name,usage_count=history.usage_count+1,last_used_at=now(),category=effective_cat where id=history.id;
      ...
      insert into grocery_item_history(device_id,name,usage_count,category,last_used_at) values(p_device_id,clean_name,1,effective_cat,now());
    end if;
