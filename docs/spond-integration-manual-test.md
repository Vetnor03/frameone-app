...
4. Messages/events sync
   ...
   - Confirm recent chats/posts and upcoming events are written to `integration_items` with provider `spond`.
5. Frame payload includes Spond items
   ...
   - Confirm Spond rows appear in the `items` array with `source: "spond"` and no firmware changes are required.
